...
namespace MiniServer.Server.Handlers
{
    public class HttpHandler : IRequestHandler
    {
        ...
        public IHttpResponse Handle(IHttpContext httpContext)
        {
            ...
            if (httpContext.Request.Path == registerPath &&
    httpContext.Request.Session.Contains(SessionStore.CurrentUserKey))
            {
                return new RedirectResponse(homePath);
            }
            else if (httpContext.Request.Path == loginPath &&
    httpContext.Request.Session.Contains(SessionStore.CurrentUserKey))
            {
                return new RedirectResponse(homePath);
            }
            else if ((httpContext.Request.Path != loginPath && httpContext.Request.Path != registerPath) &&
                !httpContext.Request.Session.Contains(SessionStore.CurrentUserKey))
            {
